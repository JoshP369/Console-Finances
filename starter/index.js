...
var change = totalChange / (finances.length - 1);

let n = totalChange/(totalMonths - 1);

let averageChange = n.toFixed(2);
...
for (let i = 1; i < finances.length; i++) {
  ...
  let difference = finances[i][1] - previousValue;
...
for (let i = 1; i < finances.length; i++) {
  ...
  const decrease = previousAmount - currentAmount;
  
  if (decrease > greatestDecreaseAmount || greatestDecreaseAmount === undefined) {
